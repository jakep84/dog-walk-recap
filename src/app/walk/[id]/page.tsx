...
async function fetchWalkForMeta(id: string) {
  ...
  const tempF = f.tempF?.integerValue ?? f.tempF?.doubleValue ?? null;
...
export async function generateMetadata({
  ...
  const data = await fetchWalkForMeta(params.id);
  ...
  return {
    ...
    openGraph: {
      ...
      description: desc,
      url: `/walk/${params.id}`,
      images: [`/walk/${params.id}/opengraph-image`],
      type: "website",
    },
    twitter: {
...
export default function Page({ params }: { params: { id: string } }) {
  return <WalkPublicClient walkId={params.id} />;
}
